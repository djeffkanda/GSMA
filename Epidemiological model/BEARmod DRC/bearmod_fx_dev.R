...
recoveryTimeStep = function(HPop, recrate_values,current_day){
  #print(recrate_values)
  recrate = subset(recrate_values,date == current_day)$recrate[1]

  HPop$nInf = round(HPop$nInf)
  #print(paste0("Day ",current_day, " recovery rate: ", recrate))
  for (i in 1:length(HPop$nInf)){
    #print(HPop$nInf[i])
    HPop$nRecoveredToday[i]= rbinom(1,HPop$nInf[i],recrate)
...
movementTimeStep = function(HPop, mobmat,day,control_df,prob_move_per_TS){
  ...
  daily_move_mat = daily_move[,is.element(names(daily_move),c("fr_pat","to_pat","move_prop"))]
  ...
  colmove = which(colnames(daily_move_mat) == "move_prop")
...
reduceContacts = function(HPop,contact_reduction_df,current_date){
  ...
  if (dim(contact_reduction_df_sub)[1] > 0){
    ...
    for (i in 1:dim(contact_reduction_df_sub)[1]){
     # print(contact_reduction_df_sub)
...
###### Master function  ####
runSim = function(HPop,pat_info,movement_reduction_df, contact_reduction_df,mobmat,day_list,recrate_values,exposerate_df,exposepd,exposed_pop_inf_prop = 0,TSinday = 1,prob_move_per_TS=0) {
  
  epidemic_curve <- data.frame(Date=as.Date(character()),
                               inf=c(),
                               stringsAsFactors=FALSE) 
  if (is.numeric(recrate_values)){
    
    recrate_values_new = data.frame(date = unique(day_list),recrate = recrate_values)
    recrate_values <- recrate_values_new
  }
